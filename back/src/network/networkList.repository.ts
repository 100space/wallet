...
import { NetWorkList } from '../schemas/networkList.schema';
import { UpdateNetworkDto } from './dto';

@Injectable()
export class NetWorkListRepository {
  constructor(
    @InjectModel(NetWorkList.name, 'local')
    private networkListModel: Model<NetWorkList>,
  ) {}

  async create(address: string) {
    const defaultData = ['ETHEREUM', 'ARBITRUM', 'POLYGON', 'GOERLI'];
    return await this.networkListModel.create({
      address,
      networkList: defaultData,
    });
  }

  async update(updateNetworkDto: UpdateNetworkDto) {
    const { address } = updateNetworkDto;
    return await this.networkListModel.findOneAndUpdate(
      { address },
      updateNetworkDto,
    );
  }
